...
**Handoff:** **Happenings only.** Search + CCFB parked. **LIVE on Pages** (`3811a42`): start demo at `happenings-listing.html`. html.to.design: use local `screens/` with `screens/img/` JPEGs (not CSS backgrounds). Optional Ventrata keys. Dev banner: `?dev=1`.
